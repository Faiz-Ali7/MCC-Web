...
const ExpenseTable = ({ filteredExpenseData }) => {
    const [searchQuery, setSearchQuery] = useState("");
    const [suggestions, setSuggestions] = useState([]);

    // Filtered Data based on Search Query
    const filteredData = filteredExpenseData.filter(item =>
    ...
    const handleSearchChange = (e) => {
        ...
        if (value.length > 0) {
            const uniqueSuggestions = [...new Set(filteredExpenseData.map(item => item.PostedBy))]
    ...
    return (
        <div className="mt-6">
            ...
            {filteredData.length > 0 ? (
                ...
                <p className="text-center text-gray-400 mt-6">No expense data available for the selected filters.</p>
            )}
        </div>
    );
};
