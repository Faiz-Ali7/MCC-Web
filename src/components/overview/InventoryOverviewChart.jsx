...
const InventoryOverviewChart = ({ filteredSalesData }) => {
	const maxValue = Math.max(...(filteredSalesData || []).map(item => item.Total), 0);
const yAxisMax = maxValue * 1.1; // Adds 10% padding
	...
	return (
		...
		>
			<h2 className='text-lg font-medium mb-4 text-gray-100 text-center'>Sales By Categories</h2>

			<div className='w-full h-96 overflow-x-auto'>
				<ResponsiveContainer width="100%" height="100%">
					<BarChart data={filteredSalesData} margin={{ right: 20, top: 10, bottom: 50 }}>
...
/>
						...
						<Bar dataKey='Total' fill='#6366F1' barSize={window.innerWidth < 768 ? 15 : 25} />
					</BarChart>
				</ResponsiveContainer>
			</div>
		</motion.div>
